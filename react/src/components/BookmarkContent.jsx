...
function ItemContent() {
  ...
  const itemIndexPresent = bookmarkList.find(bookmark => bookmark?.bookmark_id === bookmarkIdIndex)?.bookmark_item_list?.findIndex(item => item?.item_number === itemNumberIndex);
  const itemIndexLast = bookmarkList.find(bookmark => bookmark?.bookmark_id === bookmarkIdIndex)?.bookmark_item_list?.findLastIndex(item => item);

  const handleItemNumberPrev = () => itemIndexPresent > 0 && navigate(`/รายการโปรด/bookmarkIdIndex/${itemList[itemIndexPresent - 1]?.item_number}`);
  const handleItemNumberNext = () => itemIndexPresent < itemIndexLast && navigate(`/รายการโปรด/bookmarkIdIndex/${itemList[itemIndexPresent + 1]?.item_number}`);
  ...
  console.log('itemIndexPresent', itemIndexPresent);
  console.log('itemIndexLast', itemIndexLast);
